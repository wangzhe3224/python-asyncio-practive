...
async def main():
    conn = await asyncpg.connect(host='127.0.0.1', port=5432, database="zhewang")
    ...
    statements = [CREATE_BRAND_TABLE,
                  ...
                  SIZE_INSERT,
                  COLOR_INSERT]
    print(f"Creating the product database ... ")
    for statement in statements:
        status = await conn.execute(statement)
        print(status)

    print(f"Finished creating the product database.")
